...
public class UltimateButtonEditor : Editor
{
	...
	void StoreReferences ()
	{
		...
		if( !isPrefabInProjectWindow )
		{
			...
			// OLD TOUCH SIZE //
			if( ( int )targ.touchSize >= 0 )
			{
				if( ( int )targ.touchSize == 0 )
					activationRange.floatValue = 1.0f;
				else if( ( int )targ.touchSize == 1 )
					activationRange.floatValue = 1.5f;
				else if( ( int )targ.touchSize == 2 )
					activationRange.floatValue = 2.0f;

				serializedObject.FindProperty( "touchSize" ).intValue = -1;
				serializedObject.ApplyModifiedProperties();
			}

			// NEW HIERARCHY STRUCTURE //
			if( targ.sizeFolder != null && targ.buttonBase != null )
			{
				Undo.SetTransformParent( targ.buttonBase.transform, targ.transform, "Fix Older Buttons" );

				if( targ.useHighlight && targ.buttonHighlight != null )
					Undo.SetTransformParent( targ.buttonHighlight.transform, targ.buttonBase.transform, "Fix Older Buttons" );

				if( targ.showTension && targ.tensionAccent != null )
					Undo.SetTransformParent( targ.tensionAccent.transform, targ.buttonBase.transform, "Fix Older Buttons" );

				Undo.DestroyObjectImmediate( targ.sizeFolder.gameObject );

				serializedObject.FindProperty( "sizeFolder" ).objectReferenceValue = null;
				serializedObject.ApplyModifiedProperties();
			}

			// NEW BOUNDARY // OLD IMAGE STYLE //
			if( ( int )targ.imageStyle >= 0 )
			{
				serializedObject.FindProperty( "boundary" ).intValue = ( int )targ.imageStyle;
				serializedObject.FindProperty( "imageStyle" ).intValue = -1;
				serializedObject.ApplyModifiedProperties();
			}

			// NEW TENSION INPUT TRANSITION //
			if( targ.showTension )
			{
				useTension.boolValue = true;
				inputTransition.boolValue = true;
				serializedObject.FindProperty( "showTension" ).boolValue = false;
				serializedObject.ApplyModifiedProperties();
			}

			if( targ.showHighlight )
			{
				useHighlight.boolValue = true;
				serializedObject.FindProperty( "showHighlight" ).boolValue = false;
				serializedObject.ApplyModifiedProperties();
			}

			// NEW FADE INPUT TRANSITION //
			if( targ.fadeInDuration >= 0.0f || targ.fadeOutDuration >= 0.0f )
			{
				transitionTouchedDuration.floatValue = targ.fadeInDuration;
				transitionUntouchedDuration.floatValue = targ.fadeOutDuration;
				serializedObject.FindProperty( "fadeInDuration" ).floatValue = -10;
				serializedObject.FindProperty( "fadeOutDuration" ).floatValue = -10;
				serializedObject.ApplyModifiedProperties();
			}
		}
	...
	public override void OnInspectorGUI ()
	{
		...
		if( isPrefabInProjectWindow )
		{
			bool stopEditorDisplay = targ.sizeFolder != null && targ.buttonBase != null;
		...
		if( EditorPrefs.GetBool( "UB_InputSettings" ) )
		{
			...
			EditorGUI.BeginChangeCheck();
			EditorGUILayout.PropertyField( useTouchInput, new GUIContent( "Use Touch Input", "Determines if the button should use input from the EventSystem or directly calculate from the touch input on the screen." ) );
			if( EditorGUI.EndChangeCheck() )
				serializedObject.ApplyModifiedProperties();

			if( targ.useTouchInput )
				EditorGUILayout.HelpBox( "The Ultimate Button will exclusively use touch input received on the screen for calculations.", MessageType.Warning );
