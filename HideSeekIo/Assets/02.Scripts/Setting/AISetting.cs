...
public class AISetting : ScriptableObject
{
    ...
    private const string SettingFilePath = "Assets/Resources/Setting/AISetting.asset";//정확한 파일 위치,만약파

    private static AISetting _instacne;
    ...
    public static AISetting Instance
    {
        get
        {
            ...
            _instacne = Resources.Load<AISetting>(path: "AISetting");
            //에디터에서만 작용하므로 전처리기.
            ...
            if (_instacne == null)
            {
                ...
                _instacne = AssetDatabase.LoadAssetAtPath<AISetting>(SettingFilePath);

                //그래도 null이라면 만든다.
                if (_instacne == null)
                {
                    _instacne = CreateInstance<AISetting>();   //아직 메모리에만 존재
