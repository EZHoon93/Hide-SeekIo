...
namespace BeautifyEffect {
    ...
    public partial class Beautify : MonoBehaviour {
        ...
        const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint80b3";
            ...
            if (allowExtraEffects && (_lensDirt || _bloom || _anamorphicFlares || sunFlareEnabled)) {
                ...
                if (sunFlareEnabled) {
                    ...
                    if (sunScrPos.z > 0 && sunScrPos.x >= -0.1f && sunScrPos.x < 1.1f && sunScrPos.y >= -0.1f && sunScrPos.y < 1.1f) {
                        Ray ray = new Ray(currentCamera.transform.position, -_sun.transform.forward);
                    ...
                    if (flareIntensity > sunFlareCurrentIntensity) {
                        sunFlareCurrentIntensity = Mathf.Lerp(sunFlareCurrentIntensity, flareIntensity, Application.isPlaying ? 30 * Time.deltaTime * _sunFlaresRevealSpeed : 1f);
                    } else {
                        sunFlareCurrentIntensity = Mathf.Lerp(sunFlareCurrentIntensity, flareIntensity, Application.isPlaying ? 30 * Time.deltaTime * _sunFlaresHideSpeed : 1f);
                    }
